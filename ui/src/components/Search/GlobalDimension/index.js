...
export default function GlobalDimensionTable(props) {
  ...
  const pubGlobalDimension = async (payload) => {
    const response = await globalDimensionService.publishGlobalDimension(
      payload
    );
    if (response.success) {
      getData();
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, entity) => {
        return (
          <span style={{ whiteSpace: "initial" }} key={entity.name}>
            {entity.name}
          </span>
        );
      },
  ...
  return (
    <div>
      ...
      >
        <Button onClick={openAddGlobalDimension} type="primary">
