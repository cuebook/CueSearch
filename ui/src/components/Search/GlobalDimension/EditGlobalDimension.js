...
export default function EditGlobalDimension(props) {
  ...
  const getGlobalDimensionById = async (id) => {
    ...
    let values = response.values.map(
      (item) =>
        item["datasetId"] + "." + item["dataset"] + "." + item["dimension"]
    );
    let linkedDimension = response.values.map(
      (item) => item["dataset"] + "." + item["dimension"]
    );
  ...
  const editGlobalDimensionFormSubmit = async (values) => {
    ...
    const response = await globalDimensionService.editGlobalDimension(
      payload["id"],
      payload
    );
    if (response.success) {
  ...
  const initialSelectedValue =
    selectedDimension && selectedDimension.selectValues;
  // selectedLinkedDimension : Dimension that is linked with current editing global dimension
  const selectedLinkedDimension =
    selectedDimension && selectedDimension.selectLinkedDimension;
  ...
  if (selectedDimension) {
    ...
    linkedDimensionArray =
      linkedDimension &&
      linkedDimension.filter(
        (item) =>
          !selectedLinkedDimension.some((item1) => item && item == item1)
      );
    ...
    dimensionForSuggestion =
      ...
      dimensions.map((item) => (
        <Option
          ...
          key={item["dataset"] + "." + item["dimension"]}
        >
          {" "}
          {item["dataset"] + "." + item["dimension"]}{" "}
        </Option>
      ));
    ...
    dimensionForSuggestion =
      ...
      dimensionForSuggestion.filter(
        (item) =>
          !linkedDimensionArray.some((item1) => item && item.key === item1)
      );
            ...
            <div className={style.formItem} style={{ width: "100%" }}>
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Please input Global Dimension name!",
                    whitespace: true,
                  },
                ]}
              >
                <Input
                  ...
                  placeholder={"Global Dimension Name"}
                />
              </Form.Item>
  ...
  return (
    <div>
      <div className="row">
        <div>{selectedDimension ? addGlobalDimensionFormElement : null}</div>
      </div>
    </div>
  );
}
