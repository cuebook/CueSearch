...
export default function Dataset(props) {
  ...
  const getDataset = async () => {
    ...
    if (data) {
      ...
      if (params.datasetId && data.anomalyDefinitionCount)
        setViewOnlyMode(true);

      const tempColumnTypes = {};
      data.metrics &&
        data.metrics.forEach((col) => {
          tempColumnTypes[col] = "metric";
        });
      data.dimensions &&
        data.dimensions.forEach((col) => {
          tempColumnTypes[col] = "dimension";
        });
      tempColumnTypes[data.timestampColumn] = "timestamp";

      setDatasetColumnType(tempColumnTypes);
      let columns = data.metrics
        .concat(data.dimensions)
        .concat([data.timestampColumn]);
      setDatasetColumns(columns);
    }
  };

  const saveDataset = async () => {
    const metrics = Object.keys(datasetColumnType).filter(
      (col) => datasetColumnType[col] == "metric"
    );
    const dimensions = Object.keys(datasetColumnType).filter(
      (col) => datasetColumnType[col] == "dimension"
    );
    const timestamps = Object.keys(datasetColumnType).filter(
      (col) => datasetColumnType[col] == "timestamp"
    );
    ...
    if (params.datasetId)
      ...
      if (response) history.push("/datasets");
    }
  };
  ...
  const mergeColumns = (columnValue) => {
    ...
    newColumns
      .filter((col) => !datasetColumns.includes(col))
      .forEach((col) => {
        tempColumnType[col] =
          columnValue[col] === parseInt(columnValue[col], 10)
            ? "metric"
            : "dimension";
      });
  ...
  const dataTable =
    queryData && queryData.length && columns.length
      ? calculateColumnsWidth(columns, queryData, 400)
      : {};
  const queryDataTable = (
    <Table
      className={style.antdTable}
      columns={queryData ? dataTable.columns : columns}
      dataSource={queryData ? dataTable.source : queryData}
      pagination={false}
      size="small"
      bordered={true}
      scroll={{ x: queryData ? 1200 : dataTable.tableWidth, y: 450 }}
    />
  );
  ...
  const selectFieldTypeTable = (
    <Table
      columns={typeSelectorColumns}
    ...
    <>
      <div className={`xl:w-9/12 ${style.dataset}`}>
        ...
        <div className={style.selectConnection}>
          <SelectConnection
            value={datasetConnection}
            disabled={viewOnlyMode}
            onChange={setDatasetConnection}
          />
        </div>
        ...
        <div className={style.buttons}>
          <Switch
            checked={isNonRollup}
            onChange={(val) => setIsNonRollup(val)}
          />{" "}
          &nbsp; Non Roll-up Dataset
        </div>
          ...
          <>
            ...
            <div className={style.granularity}>
              ...
              <Select
                ...
                value={datasetGranularity}
                onChange={setDatasetGranlarity}
                disabled={viewOnlyMode}
              >
        ...
        <div className={style.buttons}>
          <div className={style.save}>
            <Button
              type="primary"
              onClick={saveDataset}
              disabled={viewOnlyMode || !(queryData && queryData.length)}
            >
              Save Dataset
            </Button>
          </div>
