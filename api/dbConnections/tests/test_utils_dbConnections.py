...
def testDbConnection(client, mocker):
    ...
    data = {
        "name": "test druid",
        "description": "",
        "connectionType_id": connectionType.id,
        "params": {"host": "localhost", "port": "8888"},
    }
    ...
    mixer.blend(
        "dataset.connectionParam", connectionType=postgresConnectionType, name="host"
    )
    mixer.blend(
        "dataset.connectionParam", connectionType=postgresConnectionType, name="port"
    )
    mixer.blend(
    ...
    data = {
        ...
        "description": "",
        "connectionType_id": postgresConnectionType.id,
        "params": {
            "host": "Location where postgres db is hosted",
            "database": "db name",
            "port": "25060",
            "username": "username",
            "password": "******",
    ...
    mixer.blend(
        "dataset.connectionParam", connectionType=mysqlConnectionType, name="host"
    )
    mixer.blend(
        "dataset.connectionParam", connectionType=mysqlConnectionType, name="port"
    )
    mixer.blend(
        "dataset.connectionParam", connectionType=mysqlConnectionType, name="database"
    )
    mixer.blend(
        "dataset.connectionParam", connectionType=mysqlConnectionType, name="username"
    )
    mixer.blend(
        "dataset.connectionParam", connectionType=mysqlConnectionType, name="password"
    )
    ...
    data = {
        "name": "test connection",
        "description": "",
        "connectionType_id": mysqlConnectionType.id,
        "params": {
            "host": "Location where postgres db is hosted",
            "database": "db name",
            "port": "25060",
...
def test_MSSQLConnection(client, populate_seed_data, mocker):
    ...
    paramNames = list(
        ConnectionParam.objects.filter(connectionType=connectionType).values_list(
            "name", flat=True
        )
    )
    params = {}
